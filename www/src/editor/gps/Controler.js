    ...
    constructor(par, fun) {         
        ...
        this.sob = function(s,p){
            if(s=="drag"){                
                if(self.object){
                    self.korektContr();
                    self.object.position.setPoint(p)
        ...
        this.array=[]
        var tControl=new TControl(this, this.sob)

        this.array.push(tControl);
        ...
        this.otMD = function(){
            ...
            if(self.object && to){
                ...
                to.position.copy(self.object.position)
            }
        }
        ...
        this.setObject = function(obj){
            if(this.object)this.object.otMD=undefined

            this.object=obj
            if(obj)if(obj.position==undefined)this.object=null
            ...
            if(this.object)to=this.mesh;

            if(this.object)this.object.otMD=this.otMD
            
            this.array[this._tip].setObject(to);
        }


        this.korektContr=function(){ 
            
            if(this.boolCTRL==false || self.boolsd==false){
                return
            }           
            self.boolsd=false;
            let b=0;
            if(this.object.parent && this.object.type=='Point3D' && this.object.parent.type=='Breakline'){
                b=1;    
            }
            if(this.object.parent && this.object.type=='Breakline'){
                b=1;    
            }

            if(b==1){
                let p= this.object.copy()                 
            }

            trace("#######korektContr#######",this.object,this.object.parent)    
        }
...
export class TControl {
    constructor(par, fun) {         
        ...
        //this.transformControl = new THREE.TransformControls( visi3D.camera, visi3D.renderer.domElement );
        ...
        visi3D.scene.add( this.transformControl );

        this.transformControl.addEventListener( 'objectChange', function () {
            
            
            self.fun("drag",self.object.position)
            visi3D.intRend=1;
        } );
        ...
        this.transformControl.contril=this;
        trace("!!!!!!!!!!!!!",this.transformControl)
        visi3D.addChildMouse(this.transformControl);
