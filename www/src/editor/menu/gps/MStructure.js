...
export class MStructure  {
    constructor(par, fun) {         
        this.type="MStructure";
        var self=this;
        this.par=par;
        this.fun=fun;
        ...
        this.init=function(){
            ...
            this.three=new DTStruk(this.dCont,0,0,function(s,p,p1){
                
                menuBig.setObject(p.obj.obj)
        ...
        this.object
        this.setObjBase = function(b){
            this.object=b
            if(this.three)this.three.setObjBase(b)
        }
        ...
        this.reDrag = function(){ 
            ...
            this.three.setObj(this._activeObject)
        }
...
export  function DTStruk(dCont, _x, _y, fun){
    DCont.call(this);
    this.type = 'DTStruk';
    var self = this; 
    ...
    this.setObjBase = function(b){ 
        ...
        for (var i = 0; i < this.bufferOt.length; i++) {
            if(this.bufferOt[i].obj.obj.uuid==b.uuid){
                p=i;
                break
            }
        } 
    ...
    this.mouseEvent = function(obj){ 
        ...
        if(obj.sobEvent=="mouseDown" || obj.sobEvent=="dblclick"){
            ...
            this.openCloseObj(obj, true)//!obj.isOpen); 
        }
        //Наведение курсора
    ...
    this.setObj=function(_o,_key,_key1){
        ...
        this.arrObj=[this.getAType(_o,0)];
        
        this.setArr(this.arrObj);
    }
    this.ao= menuBig.arrClass





    this.isAType=function(_o){
       
        if(_o==null)return null
        if(typeof _o =="object"){
            for (var i = 0; i < this.ao.length; i++) {
                if(this.ao[i].type==_o.type){
                    return this.ao[i];
                }
            }
        }
    ...
    var ss=0
    this.getAType=function(_o,_sah){
        
        var ooo=null;
        var oo=this.isAType(_o)
        
        ss++;  
        if(oo!==null){
            ooo={}
            ooo.text=_o.type;
            if(_o.name)ooo.text=_o.name+":"+_o.type;
            ooo.obj=_o;
            ooo.infoObj=oo; 
            ooo.arr=[];
            for(var s in _o){
                ...
                /* var oo1=this.isAType(_o[s]);
                if(oo1){
                    continue                   
                    ooo.arr.push(this.getAType(_o[s]),sah+1) 
                }*/

                //if(s=="array"){
                if(s=="children"){    
                    if(_o[s].length){
                        for (var i = 0; i < _o[s].length; i++) {
                            var oo1=this.isAType(_o[s][i]);
                            if(oo1){                   
                                ooo.arr.push(this.getAType(_o[s][i],_sah+1)) 
                            }
                        }
                    }
                }
...
DTStruk.prototype = Object.create(DCont.prototype);
DTStruk.prototype.constructor = DTStruk;
Object.defineProperties(DTStruk.prototype, {
    height: {
        set : function(value){
