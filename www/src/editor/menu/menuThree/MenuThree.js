...
export class MenuThree  {
    constructor(par, fun) {         
        ...
        this.init=function(){
            ...
            this.arrayButton[this.arrayButton.length]=this.button=new DButton(this.dCont,this.param.otstup,this.param.otstup," ",function(){
                self.index=0
                localS.object["p_MenuThree_index"]=0
                localS.save()
            })
            this.button.width=this.button.height=this.param.wh;
            this.button.scalePic=dcmParam.scalePic;
            ...
            this.arrayButton[this.arrayButton.length]=this.button1=new DButton(this.dCont,this.param.otstup,this.param.otstup*2+this.param.wh," ",function(){
                self.index=1
                localS.object["p_MenuThree_index"]=1;
                localS.save();
            })
            this.button1.width=this.button1.height=this.param.wh;
            this.button1.scalePic=dcmParam.scalePic;
            ...
            //папочки
            this.array[this.array.length]=this.mtFolder=new MTFolder(this,this.param.otstup*2+this.param.wh,this.param.otstup,function(s,p,p1){
                self.fun(s,p,p1)
            })

            this.array[this.array.length]=this.mtFolder2=new MTFolder2(this,this.param.otstup*2+this.param.wh,this.param.otstup,function(s,p,p1){
                self.fun(s,p,p1)
            });

            if(localS.object["p_MenuThree_index"]==undefined){
                localS.object["p_MenuThree_index"]=0
            }          
            

            this.index=localS.object["p_MenuThree_index"];
        }
        ...
        this.setMSBlok=function(mSBlok){
            this.mSBlok=mSBlok;
            this.mSBlok.dCont.add(this.dCont1)
            this.mSBlok.sizeWin=this.sizeWin;

            this.mSBlok.funActive=this.funActive
            this.active=this.mSBlok.active
        }
        this.funActive=function(a){
            self.active=a
        }
        var w,h;
        ...
        if (this._index != value) {
            ...
            for (var i = 0; i < this.array.length; i++) {
                if(i==value){
                    ...
                    this.arrayButton[i].color=dcmParam.colorActive
                }
                else {
                    this.array[i].active=false;
                    this.arrayButton[i].color=dcmParam.color
                }
            }
        }
    }
...
export class MTFolder  {
    constructor(par, _x,_y,fun) {         
        this.type="MTFolder";      
        var self = this; 
        this.par=par;
        this.fun=fun;
        ...
        this.init=function(){
            ...
            this.threeImp=new DTNova(this.dCont,this.param.otstup,this.param.otstup,function(s,p){ 
                trace("s,p",s,p)              
                if(p.title.indexOf(".xml")!=-1){
                    var url=self.korUrl(p);
                    self.url= url                   
                    var b=false
                    if(s=="dblclick")b=true;
                    self.fun("open",self.link+url,b) 
                    self.button.activMouse = true;                   
                }
            });


            mhbd.setPHP({tip:"getFiles1",dir:this.link, arrNot:["node_modules","hlam","date"]},function(date){
                var oo=JSON.parse(date);                
                self.threeImp.setObj(oo);               
            })


            this.panel1=new DPanel(this.dCont)
            this.panel1.height=32+this.param.otstup*2
            this.button=new DButton(this.panel1,this.param.otstup,this.param.otstup,"get",function(){
                trace(self.url,self.threeImp)
                let ll=self.link+self.url;
                $.ajax({                    
                    url: ll,
                    success: function function_name(data) {                        
                        var o1= JSON.parse(uXml.xml2json(data));
                        var p1 = uXml.json2xml(o1,"");
                        saveFileNa(p1);
                    }
                })
            })
            ...
            this.button1=new DButton(this.panel1,this.param.otstup,this.param.otstup,"set",function(e){
                
                self.saveFile(this.files[0],self.link+this.files[0].name,function(e){
                    mhbd.setPHP({tip:"getFiles1",dir:self.link, arrNot:["node_modules","hlam","date"]},function(date){
                        var oo=JSON.parse(date);                
                        self.threeImp.setObj(oo);               
                    })
                })
            })
            this.button1.startFile("xml");

            this.sizeWin();
        }
        function saveFileNa(str){
        ...
        this.saveFile=function(file,link,fun){       
        ...
        this.sizeWin = function(_w,_h){  
            ...
            this.panel.width= w;
            this.panel.height= h-this.param.otstup-this.panel1.height;
            this.threeImp.width= w-this.param.otstup*2;
            this.threeImp.height= this.panel.height-this.param.otstup*2;

            this.panel1.y=this.panel.height+this.param.otstup
            this.panel1.width = w;
            let ww=(w-this.param.otstup)/2-this.param.otstup
            this.button.width=ww;
            this.button1.width=ww;
            this.button1.x=ww+this.param.otstup*2;
        }
    }
...
export class MTFolder2  {
    constructor(par, _x,_y,fun) {         
        ...
        this.sizeWin = function(_w,_h){  
            ...
            if(this.menu)this.menu.sizeWin(w,h)
        }
...
export  function DTNova(dCont, _x, _y, fun){
    DCont.call(this);
    this.type = 'DTNova';
    var self = this; 
...
Object.defineProperties(DTNova.prototype, {
    height: {
        set : function(value){
