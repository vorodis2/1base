...
export function MVisi3D (_contentHTML, _content2d, _devas, _directional, _efect, _event3DArr, _alpha) {
	...
	this.groupZoopPoz = new THREE.Object3D();
	this.group2.add(this.groupZoopPoz);


	this.groupZoopPoz.add(this.groupObject);
	...
	this.gCamZoopPoz = new THREE.Object3D();
	this.gCam2.add(this.gCamZoopPoz);

	this.gCamZoopPoz.add(this.ggCam);
   ...
   // t.mapping = THREE.CubeUVReflectionMapping
    ...
    this.pointCamera=new THREE.Vector3()
    
    this.setPoint= function(v, bool){ 
    	
    	if(bool){
    		//v.y=0;
    		//v.z=0;
    		trace(v, bool)
    		trace(this.pointCamera)
    		//this.pointCamera.set(this.pointCamera.x-v.x,v.y,v.z)

    		//this.pointCamera.set(this.pointCamera.x-v.x,v.y,v.z)
    		this.pointCamera.set(v.x,v.y,v.z)
    	}else{
    		this.pointCamera.set(v.x,v.y,v.z)
    	}
    ...
    this.dragCPoint= function(v){ 	
    	trace(this.pointCamera)
    	//trace(this.gp)
    	

    	
    	this.gCamZoopPoz.position.x=this.pointCamera.x;
    	this.gCamZoopPoz.position.y=this.pointCamera.y;
    	this.gCamZoopPoz.position.z=this.pointCamera.z;

    	/*this.xVerh = -this.pointCamera.x;
    	this.yVerh = -this.pointCamera.y;
    	this.zVerh = this.pointCamera.z;
		//this.yVerh = o.yVerh;
		//this.zVerh = o.zVerh;*/
    	this.groupZoopPoz.position.x=this.pointCamera.x;
    	this.groupZoopPoz.position.y=this.pointCamera.z;
    	this.groupZoopPoz.position.z=this.pointCamera.y;

    	/*this.group2.position.x=this.pointCamera.x;
    	this.group1.position.y=this.pointCamera.y;
    	this.group1.position.z=this.pointCamera.z;*/

    	
    	var yy=this.pointCamera.y
    	trace(this.pointCamera.y,yy)
		aa1.position.x=this.xVerh;
		//aa1.position.y=this.pointCamera.y;
		//aa1.position.z=this.pointCamera.z;		
		/*aa1.position.z=this.pointCamera.y;
		aa1.position.y=-this.pointCamera.z;

		//aa2.position.x=this.pointCamera.x;
		aa2.position.y=-this.pointCamera.z;	
    	//this.gp2.position.copy(this.pointCamera)*/
    }
    var aa=new THREE.AxesHelper(100)
    this.scene.add(aa)

    var aa1=new THREE.AxesHelper(350)
    this.scene.add(aa1)

    var aa2=new THREE.AxesHelper(150)
    this.scene.add(aa2)

    this.fun_Positioin=function(){
    	//trace("sf",this.gCGG.position)
    	/*aa2.position.x=this.gCGG.position.x;
    	aa2.position.y=-this.gCGG.position.z;
    	aa2.position.z=this.gCGG.position.y;*/
    }
    var scall
    this.korectZoom=function(v,v1){
    	scall=v/v1;
    	this.pointCamera.x*=scall;
    	this.pointCamera.y*=scall;
    	this.pointCamera.z*=scall;
    	this.dragCPoint()
    }
/*
...
MVisi3D.prototype = {
	...
	set rotationX (v) {
		...
		if(this.fun_Positioin)this.fun_Positioin()	
	},
	get rotationX () {
